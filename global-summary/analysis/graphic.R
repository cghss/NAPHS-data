...
#############################################
## Global funnel: Final submitted barplot ###
#############################################
...
countries %>%
  ...
  mutate(pct = count/count[which(label == "Completed NAPHS")]) %>% 
  ggplot(aes(x = count, y = label, fill = label)) +
  geom_bar(stat = "identity", color = "black", fill = "#088BBE") +
  geom_text(aes(label = paste(count, 
                              ifelse(label %in% c("Published NAPHS", "Published costed line items", "Published machine readable data"),
...
#############################################
## Prior iterations of this barplot #########
## are included below ####################### 
#############################################
